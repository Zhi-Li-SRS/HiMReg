...
class CompositiveWarp(nn.Module):
    ...
    def __init__(
        ...
        if self.optimize_inverse_warp:
            ...
            inv = torch.zeros(
                [1], dtype=torch.float32, device=fixed_images.device
            )  # dummy
    ...
    def get_inverse_warp(self, n_iters: int = 50, debug: bool = False, lr=0.1):
        ...
        if self.optimize_inverse_warp:
            ...
            invwarp = compute_inverse_warp_displacement(
                self.warp.data, self.grid, invwarp, iters=20
            )
        else:
    ...
    def set_size(self, size):
        ...
        if len(self.inv.shape) > 1:
            self.inv = F.interpolate(
                self.inv.permute(*self.permute_vtoimg),
                size=size,
                mode=mode,
                align_corners=True,
            ).permute(*self.permute_imgtov)
...
class WarpAdam:
    ...
    def __init__(
        ...
        # init grid
        self.affine_init = torch.eye(self.n_dims, self.n_dims + 1, device=warp.device)[
            None
